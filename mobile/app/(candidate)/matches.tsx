...
export default function CandidateMatchesScreen() {
  ...
  useEffect(() => {
    ...
    supabase.from('matches')
      .select('*, employer_profiles(company_name), job_postings(title, pay_rate, location)')
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`mob-cand-${selected.id}`)
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          ...
          <View style={{ flex: 1, marginLeft: 12 }}>
            <Text style={styles.title} numberOfLines={1}>{selected.job_postings?.title}</Text>
            <Text style={{ color: '#64748b', fontSize: 12 }}>{selected.employer_profiles?.company_name}</Text>
          </View>
        </View>
        <FlatList
          data={messages}
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.empty}>No matches yet — keep swiping on jobs!</Text>}
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.card} onPress={() => setSelected(item)}>
            <View style={{ flex: 1 }}>
              <Text style={styles.jobName}>{item.job_postings?.title}</Text>
              <Text style={styles.companyName}>{item.employer_profiles?.company_name} · {item.job_postings?.location}</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  jobName: { color: '#fff', fontWeight: '700', fontSize: 16 },
  companyName: { color: '#64748b', fontSize: 13, marginTop: 2 },
