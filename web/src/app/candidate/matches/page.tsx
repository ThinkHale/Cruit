...
import { Match, Message, JobPosting } from '@/lib/types';
...
function MatchesList({ userId }: { userId?: string }) {
  ...
  if (matches.length === 0)
    return <Empty message="No matches yet — keep swiping on jobs!" />;
        ...
        <div key={match.id} className="bg-slate-800 rounded-2xl overflow-hidden">
          ...
          >
            <div className="text-left">
              <p className="font-bold">{match.job_postings?.title}</p>
              <p className="text-slate-400 text-sm">{match.employer_profiles?.company_name}</p>
...
function MessageThread({ matchId, userId }: { matchId: string; userId: string }) {
  ...
  useEffect(() => {
    supabase.from('messages').select('*').eq('match_id', matchId).order('created_at').then(({ data }) => setMessages(data ?? []));
    const channel = supabase
      .channel(`candidate-messages-${matchId}`)
...
function SwipeHistory({ userId }: { userId?: string }) {
  const [passed, setPassed] = useState<JobPosting[]>([]);
  const [loading, setLoading] = useState(true);
  ...
  if (loading) return <Spinner />;
  if (passed.length === 0)
    return <Empty message="No passed jobs yet." />;
      ...
      {passed.map(job => (
        <div key={job.id} className="bg-slate-800 rounded-xl p-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 text-sm">{job.employer_profiles?.company_name} · {job.location ?? '—'}</p>
          </div>
